...
class FreeplayState extends MusicBeatState
{
	...
	function listSongs(?week:Int)
	{
		...
		for (i in 0...initSonglist.length)
		{
			...
			//if (FlxG.save.data.noCopyright == 2 && copyrightedSonglist.contains(data[0]))
			//	continue;
	...
	function createSections()
	{
		...
		var weekNames = [
			"Tutorial", "Week One", "Week Two", "Week Three", "Week Four", "Week Five", "Week Six"
		];
		var iconPenis = [
			'gf', 'dad', 'spooky', 'pico', 'mom', 'parents-christmas', 'senpai'
		];
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('data/freeplaySonglist'));
