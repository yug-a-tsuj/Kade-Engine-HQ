...
class UpdaterState extends FlxState
{
    ...
    function requestUpdateInfo()
	{
		...
		var http = new haxe.Http("https://raw.githubusercontent.com/Pyram1d-dev/Kade-Engine-HQ/stable/version.downloadMe");
		...
		http.onData = function(data:String)
		{
			...
			var textWithoutVersion = data.substring(data.indexOf(';'), data.length);
			returnedData[1] = textWithoutVersion.substring(data.indexOf('-'), data.length);
